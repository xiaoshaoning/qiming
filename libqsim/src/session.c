...
/* ── Multi-unit VHDL sources ──
 *
 * vhdl_parse handles exactly one design unit per call, but real files (e.g.
 * a package declaration followed by its package body, or an entity followed
 * by its architecture) hold several.  Split such sources at unit boundaries
 * before parsing: a unit starts at a line whose first token is one of
 * `entity`, `architecture`, `package`, `configuration` (VHDL keywords, so
 * they cannot appear as identifiers at line start inside another unit).
 * Library/use context clauses are only supported before the first unit;
 * this matches the files generated by the FFT project and the test suite.
 */

static char **split_vhdl_units(const char *src, size_t *out_count)
{
    ...
    while (1) {
        ...
        if (c == '\0' || c == '\n' || c == '\r') {
            ...
            if (!in_string) {
                ...
                while (t < p && (*t == ' ' || *t == '\t')) t++;
                int is_start = 0;
                static const char *kw[] = {"entity", "package", "configuration"};
                size_t klen[] = {6, 7, 13};
                for (int k = 0; k < 4 && !is_start; k++) {
                    size_t l = klen[k];
                    if ((size_t)(p - t) > l && t[l] == ' ' &&
                        strncasecmp(t, kw[k], l) == 0) {
                        /* `package body` is a unit of its own — both must be
                         * parsed separately, so it is cut here too. */
                        is_start = 1;
                    }
                }
                if (is_start) {
                    if (count >= cap) {
                        cap *= 2;
                        size_t *no = realloc(offsets, cap * sizeof(size_t));
                        if (!no) break;
                        offsets = no;
                    }
                    offsets[count++] = (size_t)(line_start - src);
                }
            }
            while (*p == '\n' || *p == '\r') p++;
            line_start = p;
    ...
    char **units = NULL;
    if (count > 0) {
        units = calloc(count, sizeof(char *));
        if (units) {
            size_t src_len = strlen(src);
            for (size_t i = 0; i < count; i++) {
                size_t start = offsets[i];
                size_t end = (i + 1 < count) ? offsets[i + 1] : src_len;
                units[i] = malloc(end - start + 1);
                if (units[i]) {
                    memcpy(units[i], src + start, end - start);
                    units[i][end - start] = '\0';
                }
            }
        }
    ...
    free(offsets);
    *out_count = (units && count > 0) ? count : 0;
    if (count > 0 && !units) *out_count = 0;
    return units;
}
...
/* Parse one or more VHDL design units (split at unit boundaries) and add
 * them all to the session.  Returns 1 only if every unit parsed. */
static int compile_vhdl_units(qsim_session_t *sess, const char *name,
                              const char *source, size_t len)
{
    size_t nchunks = 0;
    char **chunks = split_vhdl_units(source, &nchunks);
    if (chunks && nchunks == 0) {
        free(chunks);
        chunks = NULL;
    }
    if (!chunks) {
        /* no unit-start keyword found (or alloc failure): parse whole source */
        vhdl_parse_result_t r = vhdl_parse(name, source, len);
        if (!r.success || !r.unit) {
    ...
    for (size_t i = 0; i < nchunks; i++) {
        vhdl_parse_result_t r = vhdl_parse(name, chunks[i], strlen(chunks[i]));
        if (!r.success || !r.unit) {
            log_append(sess, r.error_count > 0 ? r.errors[0].message : "VHDL parse failed");
            clear_last_diags(sess);
            if (r.error_count > 0)
        ...
        if (!add_unit(sess, r.unit)) { all_ok = 0; break; }
    }
    for (size_t i = 0; i < nchunks; i++)
        free(chunks[i]);
    free(chunks);
...
int qsim_session_compile_string(qsim_session_t *sess, const char *name, const char *source) {
    ...
    if (is_vhdl_src(source)) {
        return compile_vhdl_units(sess, name, source, strlen(source));
    } else {
...
int qsim_session_compile_file(qsim_session_t *sess, const char *path) {
    ...
    if (has_vhdl_ext(path)) {
        /* read the file and run the multi-unit path */
        FILE *f = fopen(path, "rb");
        if (!f) { log_append(sess, "cannot open file"); return 0; }
        fseek(f, 0, SEEK_END);
        long flen = ftell(f);
        fseek(f, 0, SEEK_SET);
        ...
        fclose(f);
        buf[rd] = '\0';
        int ok = compile_vhdl_units(sess, path, buf, rd);
        free(buf);
        return ok;
    } else {
